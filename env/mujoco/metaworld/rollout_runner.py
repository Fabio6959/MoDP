...
def learner_trajectory_generator(env, policy, lang="", camera_name="view_1"):
    ...
    def get_observation_dict(o, img):
        step_data = {"state": o, "image": img}  # [:3]only use the end effector positions
        return OrderedDict(step_data)

    step_data = get_observation_dict(o, img)
    # os.makedirs("save_img_details/metaworld_eval/", exist_ok=True)  # 确保目录存在
    # os.makedirs("save_data_details/metaworld_eval/", exist_ok=True)  # 确保目录存在
    for _ in range(env.max_path_length):
        a = policy.get_action(step_data)
        o, r, done, info = env.step(a)

        # flag_temp = str(info["success"])
        # img_path = os.path.join(f"save_img_details/metaworld_eval/{_}.png")
        # cv2.imwrite(img_path, img)
        # # 记录 a, o, r 的形状、最大值和最小值
        # log_entry = {
        #     "step": _,
        #     "action": {
        #         "shape": a.shape,
        ...
        #     },
        #     "done": done,
        #     "info_success": flag_temp
        # }
        # # 保存日志到 JSON 文件
        # with open(f"save_data_details/metaworld_eval/open-door-v1.json", "a") as f:
        ...
        # img_path = os.path.join(f"save_img_details/metaworld_eval/open-door-v1_img_{_}.png")
        # cv2.imwrite(img_path, img)
...
def generate_dataset_rollouts(
    ...
    for env in env_names:
        ...
        for i in range(cycles):
            ...
            step = 0
            # os.makedirs("save_img_details/metaworld/", exist_ok=True)  # 确保目录存在
            # os.makedirs("save_data_details/metaworld/", exist_ok=True)  # 确保目录存在
            try:
                for a, o, r, done, info, img in expert_trajectory_generator(env, policy):
                    # flag_temp = str(info["success"])
                    # img_path = os.path.join(f"save_img_details/metaworld/{tag}_img_{cycles}_{i}_{step}_{flag_temp}.png")
                    # cv2.imwrite(img_path, img)
                    # # 记录 a, o, r 的形状、最大值和最小值
                    # log_entry = {
                    #     "i": i,
                    ...
                    # }
                    # # 保存日志到 JSON 文件
                    # with open(f"save_data_details/metaworld/{tag}.json", "a") as f:
                    #     f.write(json.dumps(log_entry, separators=(',', ':')) + "\n")
                    eps_states.append(o)
