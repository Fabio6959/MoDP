...
class ReplayBuffer:
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        ...
        if hasattr(root["data"], "items"):
            items = root["data"].items()
        else:
            items = [(k, root["data"][k]) for k in root["data"].keys()]
        for key, value in items:
            assert (
                value.shape[0] == root["meta"]["episode_ends"][-1]
            ), f"{key} {value.shape} {root['meta']['episode_ends'].shape} "
        self.root = root
    ...
    def get_episode_idxs(self):
        ...
        def _get_episode_idxs(episode_ends):
            ep_ends = episode_ends[:] if hasattr(episode_ends, '__getitem__') else episode_ends
            result = np.zeros((ep_ends[-1],), dtype=np.int64)
            for i in range(len(ep_ends)):
                start = 0
                if i > 0:
                    start = ep_ends[i - 1]
                end = ep_ends[i]
    ...
    @property
    def n_steps(self):
        ep_ends = self.episode_ends[:] if hasattr(self.episode_ends, '__getitem__') else self.episode_ends
        if len(ep_ends) == 0:
            return 0
        return ep_ends[-1]
    ...
    def episode_lengths(self):
        ends = self.episode_ends[:] if hasattr(self.episode_ends, '__getitem__') else self.episode_ends
    ...
    def add_episode(
        ...
        for key, value in data.items():
            ...
            if key not in self.data:
                ...
                arr = self.data[key]
                # print("value.shape[1:] == arr.shape[1:]--------", value.shape, arr.shape)
    ...
    def get_episode(self, idx, copy=False):
        ep_ends = self.episode_ends[:] if hasattr(self.episode_ends, '__getitem__') else self.episode_ends
        idx = list(range(len(ep_ends)))[idx]
        start_idx = 0
        if idx > 0:
            start_idx = ep_ends[idx - 1]
        end_idx = ep_ends[idx]
