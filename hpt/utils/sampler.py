...
def create_indices(
    ...
    for i in range(len(episode_ends)):
        ...
        for idx in range(min_start, max_start + 1):
            ...
            indices.append([i, buffer_start_idx, buffer_end_idx, sample_start_idx, sample_end_idx])
...
class SequenceSampler:
    ...
    def sample_sequence(self, idx):
        eps_i, buffer_start_idx, buffer_end_idx, sample_start_idx, sample_end_idx = self.indices[idx]
        result = dict()
        for key in self.keys:
            ...
            if (sample_start_idx > 0) or (sample_end_idx < self.sequence_length) and len(sample) > 0:
                data = np.zeros(shape=(self.sequence_length,) + input_arr.shape[1:], dtype=input_arr.dtype)
                if sample_start_idx > 0 and len(sample) > 0:
